...
router.get('/', async function(req, res, next) {
  ...
  try {
    ...
    res.status(500).json({ success: false, message: '獲取用戶列表錯誤: ' + error.message });
  } finally {
    if (client) {
      await client.close();
    }
...
router.put('/:phoneNo', async function(req, res, next) {
  ...
  try {
    ...
    if (!user) {
      ...
      return res.status(404).json({ success: false, message: `找不到用戶 (${phoneNo})，無法更新資料` });
    }
    ...
    res.status(500).json({ success: false, message: '更新用戶資料錯誤: ' + error.message });
  } finally {
    await client.close();
  }
});
